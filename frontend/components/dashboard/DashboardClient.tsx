...
import { generateExecutiveSummary } from '@/lib/services/geminiService';
...
const DashboardClient: React.FC<DashboardClientProps> = ({
  ...
  const activeProjects = useMemo(() => {
    return clients.reduce((acc, c) => acc + (c.projects?.length || 0), 0);
  }, [clients]);

  const winRate = useMemo(() => {
    const wonLeads = leads.filter((l) => l.stage === PipelineStage.WON);
    const lostLeads = leads.filter((l) => l.stage === PipelineStage.LOST);
  ...
  const revenueByService = useMemo(() => {
    ...
    wonLeads.forEach((l) => {
      const type = l.serviceType || 'Other';
      serviceRevenueMap[type] = (serviceRevenueMap[type] || 0) + l.value;
    });
