...
export const analyzeLeadRisk = async (
  ...
  if (apiKey === 'dummy-key') {
    return {
      ...
      recommendations: ['Follow up within 48 hours', 'Prepare proposal'],
    };
  }
...
export const generateClientHealthReport = async (
  ...
  try {
    const revenue = client.totalRevenue || client.lifetimeRevenue || 0;
    const prompt = `
...
export const askCRM = async (
  ...
  try {
    ...
    const leadsSummary = contextData.leads
      .map(
        (l) =>
          `${l.name} (${l.company}): $${l.value}, Stage: ${l.stage}`
      )
      .join('\n');
