...
export interface DashboardMetrics {
  ...
  winRate: number;
  avgDealSize: number;
  funnelDropOff: { stage: string; count: number; dropOffRate: number }[];
...
export interface RevenueBreakdown {
  ...
  monthlyRevenue: number;
  quarterlyRevenue: number;
  byClient: { clientId: string; clientName: string; revenue: number }[];
  byProject: { projectId: string; projectName: string; revenue: number }[];
  concentration: {
    topClientPercentage: number;
    top5ClientsPercentage: number;
    isHighRisk: boolean;
...
export interface ProjectAnalytics {
  ...
  atRisk: { projectId: string; projectName: string; reason: string }[];
}
...
export const dashboardApi = {
  async getMetrics(
    ...
    const response = await fetch(`${API_URL}/dashboard/metrics?period=${period}`, {
      headers,
      credentials: 'include',
    });
