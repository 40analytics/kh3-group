...
export class ClientMetricsService {
   ...
   */
  async calculateMetrics(clientId: string, clientCreatedAt: Date): Promise<ClientMetrics> {
    const now = new Date();
    ...
    const daysSinceLastContact = latestActivity
      ...
      : Math.floor((now.getTime() - clientCreatedAt.getTime()) / (1000 * 60 * 60 * 24));
    ...
    const avgProjectValue = totalProjectCount > 0 ? totalRevenue / totalProjectCount : 0;
   ...
   */
  detectHealthFlags(metrics: ClientMetrics, lifetimeRevenue?: number): ClientHealthFlag[] {
   ...
   */
  generateSuggestedActions(flags: ClientHealthFlag[], metrics: ClientMetrics): string[] {
   ...
   */
  determineHealthStatus(engagementScore: number, activeProjectCount: number): string {
